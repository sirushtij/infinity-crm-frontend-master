...
import { saveVarient } from "../../../services/varient.service";
import "../../../assets/scss/pages/data-list.scss";

const VarientForm = ({ data, show, handleSidebar, getVarientsData }) => {
  const formFields = {
    id: "",
    name: "",
    status: true,
  ...
  const handleSubmit = async (values) => {
    ...
    const res = await saveVarient(fData);
    if (res.status === 200) {
      handleSidebar(false);
      getVarientsData();
    }
        ...
        <Form>
          ...
          >
            <div className="data-list-sidebar-header mt-2 px-2 d-flex justify-content-between">
              <h4>{data !== null ? "Update Varient" : "Add New Varient"}</h4>
