...
class SideMenuContent extends React.Component {
  ...
  componentDidMount() {
    ...
    if (role["name"] === "Admin" && role["id"] === 1) {
      this.setState({ sidebarItems: navigationConfig });
    } else {
      const permissionByName = {};
      permissions.forEach((item) => {
        permissionByName[item.permission_detail.name] = item;
      });

      const tempItems = navigationConfig.filter((item) => {
        if (item.general) return true;
        else if (
          item.permission &&
          permissionByName[item.permission] &&
          permissionByName[item.permission]["read"]
        )
          return true;
        return false;
      });
      this.setState({ sidebarItems: tempItems });
    }
  ...
  render() {
    ...
    const menuItems = this.state.sidebarItems.map((item) => {
